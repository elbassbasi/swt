...
void w_theme_draw_background(w_theme *theme,w_themedata *data, w_graphics *gc, w_rect *bounds){
    if (theme != 0 && theme->clazz != 0) {
        theme->clazz->draw_background(theme,data,gc,bounds);
    }
}
void w_theme_draw_focus(w_theme *theme,w_themedata *data, w_graphics *gc, w_rect *bounds){
    if (theme != 0 && theme->clazz != 0) {
        theme->clazz->draw_focus(theme,data,gc,bounds);
    }
}
void w_theme_draw_image(w_theme *theme,w_themedata *data, w_graphics *gc, w_rect *bounds,w_image *image, int flags){
    if (theme != 0 && theme->clazz != 0) {
        theme->clazz->draw_image(theme,data,gc,bounds,image,flags);
    }
}
void w_theme_draw_text(w_theme *theme,w_themedata *data, w_graphics *gc, w_rect *bounds, const char *text, int length, int flags){
    if (theme != 0 && theme->clazz != 0) {
...
int w_theme_get_selection(w_theme *theme,w_themedata *data, w_point *offset, w_rect *bounds){
    if (theme != 0 && theme->clazz != 0) {
        return theme->clazz->get_selection(theme,data,offset,bounds);
    }else return 0;
}
int w_theme_hit_background(w_theme *theme,w_themedata *data, w_point *position, w_rect *bounds){
    if (theme != 0 && theme->clazz != 0) {
        return theme->clazz->hit_background(theme,data,position,bounds);
    }else return 0;
}
void w_theme_measure_text(w_theme *theme,w_themedata *data,w_graphics *gc, w_rect *bounds,w_rect *result, const char *text, size_t length,
                          int flags){
    if (theme != 0 && theme->clazz != 0) {
        theme->clazz->measure_text(theme,data,gc,bounds,result,text,length,flags);
    }
}
...
w_font* w_theme_get_font(w_theme *theme){
    if (theme != 0 && theme->clazz != 0) {
        ...
        return w_toolkit_get_system_font(w_toolkit_get_platform());
    }
}
...
wuchar themeclasses0[_W_THEME_CLASS_LAST] = {
        ...
        W_THEME_CLASS_BROWSER,
        W_THEME_CLASS_TREE,
        W_THEME_CLASS_TABFOLDER,
...
wuint w_theme_internal_class_id(wuint clazz){
    ...
    if(clazz >= W_THEME_CLASS_LAST) return 0;
    else return themeclasses[clazz];
}