...
wresult w_browser_get_javascript_enabled(w_browser *browser) {
	if (W_WIDGET_CHECK(browser)) {
...
wresult w_browser_is_back_enabled(w_browser *browser) {
	if (W_WIDGET_CHECK(browser)) {
		return W_BROWSER_GET_CLASS(browser)->is_back_enabled(browser);
	} else
		return W_FALSE;
}
wresult w_browser_is_forward_enabled(w_browser *browser) {
	if (W_WIDGET_CHECK(browser)) {
...
wresult w_browser_set_javascript_enabled(w_browser *browser, int enabled) {
	if (W_WIDGET_CHECK(browser)) {
