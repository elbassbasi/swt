...
void _w_button_select_radio(w_widget *widget, struct _w_event_platform *e,
	 ...
	 */
	_w_fixed *fixed = (_w_fixed*) reserved->topHandle(widget, reserved);
...
wresult _w_button_set_text(w_button *button, const char *text, size_t length) {
	...
	struct _w_widget_handles handles;
	_mnemonics_fix(&mn, text, length, W_TRUE);
	memset(&handles, 0, sizeof(handles));
	handles.handle = _W_WIDGET(button)->handle;
	_w_widget_get_handles(&handles);
	if (handles.label != 0) {
...
gboolean _gtk_button_clicked(struct w_widget *widget,
	...
	if ((_W_WIDGET(widget)->style & W_RADIO) != 0) {
		...
		if ((_W_WIDGET(parent)->style & W_NO_RADIO_GROUP) != 0) {
			...
			_w_button_select_radio(widget, e, reserved);
		}
	} else {
...
void _w_button_hook_events(w_widget *widget,
	...
	if (_W_BUTTON_RESERVED(reserved)->signal_clicked_id == 0) {
		_W_BUTTON_RESERVED(reserved)->signal_clicked_id = g_signal_lookup(
				"clicked", gtk_button_get_type());
	}
	_w_widget_connect(_W_WIDGET(widget)->handle, SIGNAL_CLICKED,
	_W_BUTTON_RESERVED(reserved)->signal_clicked_id, FALSE);
	/*	if (labelHandle != 0) {
...
wresult _w_button_create(w_widget *widget, w_widget *parent, int style,
	...
	_w_button_hook_events(widget, _w_widget_get_reserved(widget));
	...
	if (wrap) {
		...
		if (wHint != W_DEFAULT) {
			pango_layout_set_width(labelLayout,
					w_max(1, (wHint - imageWidth - trimWidth)) * PANGO_SCALE);
		} else {
		...
		e->size->height +=
				hHint == W_DEFAULT ?
						w_max(w_max(imageHeight, indicatorHeight), h)
								+ trimHeight :
						hHint;
	} else {
		_w_control_compute_native_size(widget, handles.handle, e, reserved);
	}
	if ((_W_WIDGET(widget)->style & (W_CHECK | W_RADIO)) != 0) {
