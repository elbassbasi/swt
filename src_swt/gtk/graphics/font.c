...
wresult w_fontdata_get_locale(w_fontdata *fontdata, w_alloc text,void* user_data) {
    return W_FALSE;
}
wresult w_fontdata_get_name(w_fontdata *fontdata, w_alloc text,void* user_data) {
    size_t  l = strlen(_W_FONTDATA(fontdata)->family);
    char* ret = 0;
    size_t newl = text(user_data,l+1,(void**)&ret);
    if(ret != 0){
...
wresult w_font_get_fontdata(w_font *font, w_fontdata *fontdata) {
	...
	w_fontdata_set_name(fontdata,
			pango_font_description_get_family(_W_FONT(font)->handle));
