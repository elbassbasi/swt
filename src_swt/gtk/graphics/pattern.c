...
wresult w_pattern_create_from_image(w_pattern *pattren, w_image *image) {
	if (image == 0) return W_ERROR_NULL_ARGUMENT;
	if (!w_image_is_ok(image)) return W_ERROR_INVALID_ARGUMENT;
    ...
    if(surface == 0) return W_ERROR_NO_HANDLES;
    ...
    cairo_destroy(cairo);
	cairo_surface_destroy(surface);
	if (_W_PATTERN(pattren)->handle == 0) return W_ERROR_NO_HANDLES;
...
wresult w_pattern_create(w_pattern *pattren, float x1, float y1, float x2,
	...
	if (_W_PATTERN(pattren)->handle == 0) return W_ERROR_NO_HANDLES;
	cairo_pattern_add_color_stop_rgba(_W_PATTERN(pattren)->handle,0,(double)W_RED(color1)/(double)0xFFFF,(double)W_GREEN(color1)/(double)0xFFFF,(double)W_BLUE(color1)/(double)0xFFFF,(double)W_ALPHA(color1)/(double)0xFFFF);
    cairo_pattern_add_color_stop_rgba(_W_PATTERN(pattren)->handle,1,(double)W_RED(color2)/(double)0xFFFF,(double)W_GREEN(color2)/(double)0xFFFF,(double)W_BLUE(color2)/(double)0xFFFF,(double)W_ALPHA(color2)/(double)0xFFFF);
    cairo_pattern_set_extend(_W_PATTERN(pattren)->handle, CAIRO_EXTEND_REPEAT);
	return W_TRUE;
}
#endif
