...
wresult _w_dragsource_create(w_widget *dragsource, w_widget *control,
		wuint64 style, w_widget_post_event_proc proc) {
	if (control == 0)
		return W_ERROR_NULL_ARGUMENT;
	if (!w_widget_is_ok(control))
	...
	if (gtk_toolkit->closures[SIGNAL_DRAG_DATA_GET].widget_closures == 0) {
		gtk_toolkit->closures[SIGNAL_DRAG_DATA_GET].widget_closures =
				g_cclosure_new((GCallback) _w_dragsource_drag_get_data,
						(gpointer) (long) SIGNAL_DRAG_DATA_GET, 0);
		gtk_toolkit->closures[SIGNAL_DRAG_END].widget_closures = g_cclosure_new(
				(GCallback) _w_dragsource_drag_end,
				(gpointer) (long) SIGNAL_DRAG_END, 0);
		gtk_toolkit->closures[SIGNAL_DRAG_DATA_DELETE].widget_closures =
				g_cclosure_new((GCallback) _w_dragsource_drag_data_delete,
						(gpointer) (long) SIGNAL_DRAG_DATA_DELETE, 0);
	}
	_w_widget_connect(_W_WIDGET(control)->handle, SIGNAL_DRAG_DATA_GET, 0,
	FALSE);
	_w_widget_connect(_W_WIDGET(control)->handle, SIGNAL_DRAG_END, 0, FALSE);
	_w_widget_connect(_W_WIDGET(control)->handle, SIGNAL_DRAG_DATA_DELETE, 0,
	FALSE);

	W_CONTROL_GET_CLASS(control)->create_dragsource_effect(W_CONTROL(control),
			&_W_DRAGSOURCE(dragsource)->effect);
	_W_CONTROL(control)->dragsource = W_DRAGSOURCE(dragsource);
	_W_DRAGSOURCE(dragsource)->control = W_CONTROL(control);
	_W_WIDGET(dragsource)->post_event = proc;
	return W_TRUE;
}
w_control* _w_dragsource_get_control(w_dragsource *dragsource) {
	return _W_DRAGSOURCE(dragsource)->control;
}
w_dragsource_effect* _w_dragsource_get_effect(w_dragsource *dragsource) {
	return &_W_DRAGSOURCE(dragsource)->effect;
}
...
int _w_dragsource_dragdetect(w_widget *widget, w_event *e) {
	...
	_w_widget_send_event(W_WIDGET(widget), (w_event*) &event);
	size_t transferAgents_length = 0;
	...
	if (GTK_VERSION >= VERSION(3, 10, 0)) {
		...
		_W_WIDGET(control)->handle,
		_W_DRAGSOURCE(widget)->targetList, actions, 1, 0, -1, -1);
	} else {
		context = gtk_drag_begin(_W_WIDGET(control)->handle,
		_W_DRAGSOURCE(widget)->targetList, actions, 1, 0);
	}
...
int _w_dragsource_post_event(w_widget *widget, w_event *e) {
	switch (e->type) {
	case W_EVENT_DRAGDETECT:
		return _w_dragsource_dragdetect(widget, e);
		break;
	case W_EVENT_DISPOSE:
		return _w_dragsource_dispose_0(widget);
		break;
	}
	return W_FALSE;
}
void _w_dragsource_dispose(w_widget *widget) {
...
gboolean _w_droptarget_set_event_data(w_droptarget *target,
	...
	GdkWindow *window = gtk_widget_get_window(_W_WIDGET(control)->handle);
...
gboolean _w_droptarget_drag_data_received(GtkWidget *widget,
	...
	_w_widget_send_event(W_WIDGET(target), (w_event*) &event);
...
wresult _w_droptarget_create(w_widget *droptarget, w_widget *control,
		wuint64 style, w_widget_post_event_proc proc) {
	if (control == 0)
		return W_ERROR_NULL_ARGUMENT;
	if (!w_widget_is_ok(control))
	...
	if (gtk_toolkit->closures[SIGNAL_DRAG_MOTION].widget_closures == 0) {
		gtk_toolkit->closures[SIGNAL_DRAG_MOTION].widget_closures =
				g_cclosure_new((GCallback) _w_droptarget_drag_motion,
						(gpointer) (long) SIGNAL_DRAG_MOTION, 0);
		gtk_toolkit->closures[SIGNAL_DRAG_LEAVE].widget_closures =
				g_cclosure_new((GCallback) _w_droptarget_drag_leave,
						(gpointer) (long) SIGNAL_DRAG_LEAVE, 0);
		gtk_toolkit->closures[SIGNAL_DRAG_DATA_RECEIVED].widget_closures =
				g_cclosure_new((GCallback) _w_droptarget_drag_data_received,
						(gpointer) (long) SIGNAL_DRAG_DATA_RECEIVED, 0);
		gtk_toolkit->closures[SIGNAL_DRAG_DROP].widget_closures =
				g_cclosure_new((GCallback) _w_droptarget_drag_drop,
						(gpointer) (long) SIGNAL_DRAG_DROP, 0);
	}
	_w_widget_connect(_W_WIDGET(control)->handle, SIGNAL_DRAG_MOTION, 0, FALSE);
	_w_widget_connect(_W_WIDGET(control)->handle, SIGNAL_DRAG_LEAVE, 0, FALSE);
	_w_widget_connect(_W_WIDGET(control)->handle, SIGNAL_DRAG_DATA_RECEIVED, 0,
	FALSE);
	_w_widget_connect(_W_WIDGET(control)->handle, SIGNAL_DRAG_DROP, 0, FALSE);
	W_CONTROL_GET_CLASS(control)->create_droptarget_effect(W_CONTROL(control),
			&_W_DROPTARGET(droptarget)->effect);
	...
	_W_WIDGET(droptarget)->post_event = proc;
	return W_TRUE;
}
...
w_droptarget_effect* _w_droptarget_get_effect(w_droptarget *droptarget) {
	return &_W_DROPTARGET(droptarget)->effect;
}
...
int _w_droptarget_dispose_0(w_widget *widget) {
	...
	if (control == 0)
		return W_FALSE;
	g_signal_handler_disconnect(_W_WIDGET(control)->handle,
	...
	if (_W_DROPTARGET(widget)->transfer_length != 0)
		gtk_drag_dest_unset(_W_WIDGET(control)->handle);
	_W_WIDGET(widget)->clazz = 0;
	return W_TRUE;
}
int _w_droptarget_post_event(w_widget *widget, w_event *e) {
	if (e->type == W_EVENT_DISPOSE) {
		return _w_droptarget_dispose_0(widget);
	}
	return W_FALSE;
}
void _w_droptarget_dispose(w_widget *widget) {
...
void _w_droptarget_class_init(struct _w_droptarget_class *clazz) {
	...
	clazz->get_effect = _w_droptarget_get_effect;
