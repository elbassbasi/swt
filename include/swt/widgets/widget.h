...
struct _w_widgetdata_class {
	w_toolkit *toolkit;
	wbool (*is_ok)(w_widgetdata *obj);
	void (*close)(w_widgetdata *obj);
	void (*copy)(w_widgetdata *from, w_widgetdata *to);
	wbool (*equals)(w_widgetdata *obj1, w_widgetdata *obj2);
};
struct _w_item_class {
	struct _w_widgetdata_class widgetdata;
	w_widget* (*get_parent_widget)(w_item *item);
	void* (*get_data)(w_item *item);
	int (*get_index)(w_item *item);
	wresult (*get_text)(w_item *item, w_alloc alloc,void* user_data);
	wresult (*set_data)(w_item *item, void *data);
	wresult (*set_text)(w_item *item, const char *string);
};
...
SWT_PUBLIC w_widget_post_event_proc w_widget_set_post_event(w_widget *widget,
		w_widget_post_event_proc post_event);
SWT_PUBLIC int w_widget_send_event(w_widget *widget, w_event *event);
SWT_PUBLIC void w_widget_set_id(w_widget *widget, wuint id);
...
SWT_PUBLIC int w_widget_default_post_event(w_widget *widget, w_event *e);

SWT_PUBLIC void w_widgetdata_init(w_widgetdata *obj);
SWT_PUBLIC wbool w_widgetdata_is_ok(w_widgetdata *obj);
SWT_PUBLIC void w_widgetdata_close(w_widgetdata *obj);
SWT_PUBLIC void w_widgetdata_copy(w_widgetdata *from, w_widgetdata *to);
SWT_PUBLIC wbool w_widgetdata_equals(w_widgetdata *obj1, w_widgetdata *obj2);
SWT_PUBLIC void w_widgetdata_init_copy(w_widgetdata *from, w_widgetdata *to);

SWT_PUBLIC w_widget* w_item_get_parent_widget(w_item *item);
SWT_PUBLIC void* w_item_get_data(w_item *item);
SWT_PUBLIC int w_item_get_index(w_item *item);
SWT_PUBLIC wresult w_item_get_text(w_item *item,w_alloc alloc,void* user_data);
SWT_PUBLIC size_t w_item_get_text_length(w_item *item, char *text,
		size_t length);
SWT_PUBLIC wresult w_item_set_data(w_item *item, void *data);
SWT_PUBLIC wresult w_item_set_text(w_item *item, const char *string);

SWT_PUBLIC int w_widget_check(w_widget *widget);
SWT_PUBLIC int w_widgetdata_check(w_widgetdata *obj);
//WIDGETDATA
