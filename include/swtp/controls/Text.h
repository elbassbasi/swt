 ...
 * </p>
 */
class SWTP_PUBLIC WText: public WScrollable {
public:
	WText() {
	 ...
	 * @see Widget#getStyle
	 */
	WText(WComposite *parent, wuint64 style) {
		Create(parent, style);
	}
	/**
	 * Constructs a new instance of this class given its parent
...
public:
	wresult _append(const char *text, size_t length,int enc) {
		return w_text_append(W_TEXT(this), text, length,enc);
	}
	wresult _clear_selection() {
		return w_text_clear_selection(W_TEXT(this));
	}
	wresult _copy() {
		return w_text_copy(W_TEXT(this));
	}
	wresult _cut() {
		return w_text_cut(W_TEXT(this));
	}
	wresult _get_caret_line_number() {
		return w_text_get_caret_line_number(W_TEXT(this));
	}
	wresult _get_caret_location(WPoint *location) {
		return w_text_get_caret_location(W_TEXT(this), (w_point*) location);
	}
	wresult _get_caret_position(int enc) {
		return w_text_get_caret_position(W_TEXT(this),enc);
	}
	wresult _get_char_count(int enc) {
		return w_text_get_char_count(W_TEXT(this),enc);
	}
	wresult _get_doubleclick_enabled() {
		return w_text_get_doubleclick_enabled(W_TEXT(this));
	}
	wresult _get_echo_char() {
		return w_text_get_echo_char(W_TEXT(this));
	}
	wresult _get_editable() {
		return w_text_get_editable(W_TEXT(this));
	}
	wresult _get_line_count() {
		return w_text_get_line_count(W_TEXT(this));
	}
	wresult _get_line_delimiter() {
		return w_text_get_line_delimiter(W_TEXT(this));
	}
	wresult _get_line_height() {
		return w_text_get_line_height(W_TEXT(this));
	}
	wresult _get_message(w_alloc alloc, void *user_data,int enc) {
		return w_text_get_message(W_TEXT(this), alloc, user_data,enc);
	}
	wresult _get_position(const WPoint *point,int enc) {
		return w_text_get_position(W_TEXT(this), (w_point*) point,enc);
	}
	wresult _get_selection(WRange *range,int enc) {
		return w_text_get_selection(W_TEXT(this), (w_range*) range,enc);
	}
	wresult _get_selection_count(int enc) {
		return w_text_get_selection_count(W_TEXT(this),enc);
	}
	wresult _get_selection_text(w_alloc alloc, void *user_data,int enc) {
		return w_text_get_selection_text(W_TEXT(this), alloc, user_data,enc);
	}
	wresult _get_tabs() {
		return w_text_get_tabs(W_TEXT(this));
	}
	wresult _get_text(w_alloc alloc, void *user_data,int enc) {
		return w_text_get_text(W_TEXT(this), alloc, user_data,enc);
	}
	wresult _get_text_range(const WRange *range, w_alloc alloc,
			void *user_data,int enc) {
		return w_text_get_text_range(W_TEXT(this), (w_range*) range, alloc,
				user_data,enc);
	}
	wresult _get_text_range(const WRange &range, w_alloc alloc,
			void *user_data,int enc) {
	...
	wresult _get_text_limit(int enc) {
		return w_text_get_text_limit(W_TEXT(this),enc);
	}
	wresult _get_top_index() {
		return w_text_get_top_index(W_TEXT(this));
	}
	wresult _get_top_pixel() {
		return w_text_get_top_pixel(W_TEXT(this));
	}
	wresult _insert(const char *text, size_t length,int enc) {
		return w_text_insert(W_TEXT(this), text, length,enc);
	}
	wresult _paste() {
		return w_text_paste(W_TEXT(this));
	}
	wresult _select_all() {
		return w_text_select_all(W_TEXT(this));
	}
	wresult _set_doubleclick_enabled(bool doubleClick) {
		return w_text_set_doubleclick_enabled(W_TEXT(this), doubleClick);
	}
	wresult _set_echochar(int echo) {
		return w_text_set_echochar(W_TEXT(this), echo);
	}
	wresult _set_editable(bool editable) {
		return w_text_set_editable(W_TEXT(this), editable);
	}
	wresult _set_message(const char *message, size_t length,int enc) {
		return w_text_set_message(W_TEXT(this), message, length,enc);
	}
	wresult _set_selection(int start,int enc) {
		return w_text_set_selection(W_TEXT(this), start,enc);
	}
	wresult _set_selection_range(const WRange *selection,int enc) {
		return w_text_set_selection_range(W_TEXT(this), (w_range*) selection,enc);
	}
	wresult _set_tabs(int tabs) {
		return w_text_set_tabs(W_TEXT(this), tabs);
	}
	wresult _set_text(const char *text, size_t length,int enc) {
		return w_text_set_text(W_TEXT(this), text, length,enc);
	}
	wresult _set_text_limit(int limit) {
		return w_text_set_text_limit(W_TEXT(this), limit);
	}
	wresult _set_top_index(int index) {
		return w_text_set_top_index(W_TEXT(this), index);
	}
	wresult _show_selection() {
		return w_text_show_selection(W_TEXT(this));
	}
private:
	void *handles[(sizeof(w_text) - sizeof(w_scrollable)) / sizeof(void*)];
};
