 ...
 * </dl>
 */
class WDropTarget: public WWidget {
public:
	WDropTarget() {
	...
	bool SetTransfer(WTransfer **transferAgents, size_t length) {
		return set_transfer(transferAgents, length) > 0;
	}
...
public:
	WResult create(WToolkit *toolkit, WControl *control, wuint64 style,
		...
		return w_droptarget_create(W_DROPTARGET(this), (w_toolkit*) toolkit,
				(w_control*) control, style, post_event);
	}
	WControl* get_control() {
		return (WControl*) w_droptarget_get_control(W_DROPTARGET(this));
	}
	WDropTargetEffect* get_effect() {
 ...
 * </dl>
 */
class WDragSource: public WWidget {
public:
	WDragSource() {
...
public:
	WResult create(WToolkit *toolkit, WControl *control, wuint64 style,
			w_widget_post_event_proc post_event) {
		return w_dragsource_create(W_DRAGSOURCE(this), (w_toolkit*) toolkit,
				(w_control*) control, style, post_event);
	}
	WControl* get_control() {
