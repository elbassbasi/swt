...
public:
	...
	WResult _scroll(const WPoint &dest, const WRect &rect, wbool all) {
...
public:
	WResult _get_bounds(WPoint* location,WSize* size) {
		return w_caret_get_bounds(W_CARET(this),(w_point*)location,(w_size*)size);
	}
	...
	WResult _set_bounds(const WPoint* location,const WSize* size) {
		return w_caret_set_bounds(W_CARET(this),(w_point*)location,(w_size*)size);
	}
