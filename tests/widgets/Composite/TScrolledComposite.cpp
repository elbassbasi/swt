...
bool TScrolledComposite::OnLayout(WEvent &e) {
	...
	if (this->expandHorizontal) {
		contentRect.width = w_max(this->minWidth, hostRect.width);
	}
	if (this->expandVertical) {
		contentRect.height = w_max(this->minHeight, hostRect.height);
	}
	WGraphics gc;
	...
	if (hBar.IsOk()) {
		...
		hBar.SetThumb(w_min(contentRect.width, hostRect.width));
	...
	if (vBar.IsOk()) {
		...
		vBar.SetThumb(w_min(contentRect.height, hostRect.height));
