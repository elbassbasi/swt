...
wbool wc_button_get_grayed(w_button *button) {
    struct wc_button_priv *priv = wc_control_get_priv(W_CONTROL(button));
    return priv->state & W_GRAYED != 0;
}
wbool wc_button_get_image(w_button *button, w_image *image) {
}
wbool wc_button_get_selection(w_button *button) {
    struct wc_button_priv *priv = wc_control_get_priv(W_CONTROL(button));
...
wresult wc_button_set_grayed(w_button *button, wbool grayed) {
    struct wc_button_priv *priv = wc_control_get_priv(W_CONTROL(button));
    if(grayed){
        priv->state |= W_GRAYED;
    }else{
...
wresult wc_button_set_selection(w_button *button, wbool selected) {
    struct wc_button_priv *priv = wc_control_get_priv(W_CONTROL(button));
    if(selected){
