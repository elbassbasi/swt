...
wresult w_image_create_from_stream(w_image *image, w_stream *stream) {
	GInputStream *in = _w_input_stream_new(stream);
	GError *err = NULL;
	_W_IMAGE(image)->pixbuf = gdk_pixbuf_new_from_stream(in, NULL, &err);
	g_object_unref(in);
	if (_W_IMAGE(image)->pixbuf == 0) {
		return W_ERROR_NO_HANDLES;
	}
...
wresult w_image_create_from_buffer(w_image *image, const void *buffer,
	...
	w_image_dispose(image);
	GError *error = 0;
	GdkPixbuf *pixbuf = gdk_pixbuf_new_from_inline(size, (guint8*) buffer,
	TRUE, &error);
	if (error != 0) {
		g_error_free(error);
	}
...
wresult w_image_save_to_file(w_image *image, const char *file,
		const char *type) {
	GError *error = 0;
	gboolean ret = gdk_pixbuf_savev(_W_IMAGE(image)->pixbuf, file, type, 0, 0,
			&error);
	return ret;
}
wresult w_image_save_to_buffer(w_image *image, w_alloc alloc, void *user_data,
	...
	gchar *buffer = 0;
	gsize buffer_size = 0;
	gboolean ret = gdk_pixbuf_save_to_bufferv(_W_IMAGE(image)->pixbuf, &buffer,
			&buffer_size, type, 0, 0, &error);
	if (ret) {
		void *data = 0;
		size_t newsize;
		newsize = alloc(user_data, buffer_size, &data);
		if (data != 0) {
			memcpy(data, buffer, MIN(newsize, buffer_size));
		}
	}
	if (buffer != 0) {
		g_free(buffer);
...
wresult w_image_save_to_stream(w_image *image, w_stream *stream,
		const char *type) {
	GError *error = 0;
	GOutputStream *out = _w_output_stream_new(stream);
	gboolean ret = gdk_pixbuf_save_to_streamv(_W_IMAGE(image)->pixbuf, out,
			type, 0, 0, 0, &error);
	g_object_unref(out);
	return ret;
}
...
wresult _w_toolkit_image_to_disabled(w_toolkit *toolkit, w_image *image,
		w_image *destimage) {
	return w_image_to_disabled(image,
			w_toolkit_get_system_color(toolkit, W_COLOR_WIDGET_BACKGROUND),
			w_toolkit_get_system_color(toolkit, W_COLOR_WIDGET_NORMAL_SHADOW),
			destimage);
}
...
void w_surface_dispose(w_surface *bitmap) {
	if (bitmap != 0 && _W_SURFACE(bitmap)->surface != 0) {
		cairo_surface_destroy(_W_SURFACE(bitmap)->surface);
		_W_SURFACE(bitmap)->surface = 0;
	}
}
wresult w_surface_create(w_surface *bitmap, w_size *size) {
	w_surface_dispose(bitmap);
#if USE_CAIRO
	_W_SURFACE(bitmap)->surface = cairo_image_surface_create(
			CAIRO_FORMAT_ARGB32, size->width, size->height);
	if ( _W_SURFACE(bitmap)->surface == 0) {
		return W_ERROR_NO_HANDLES;
	}
	return W_TRUE;
#else
...
wresult w_surface_get_image(w_surface *bitmap, w_image *destimage) {
	...
	if (destimage == 0)
		return W_ERROR_NULL_ARGUMENT;
	w_image_dispose(destimage);
	_W_IMAGE(destimage)->pixbuf = gdk_pixbuf_get_from_surface(
	_W_SURFACE(bitmap)->surface, 0, 0,
			cairo_image_surface_get_width(_W_SURFACE(bitmap)->surface),
			cairo_image_surface_get_height(_W_SURFACE(bitmap)->surface));
	if (_W_IMAGE(destimage)->pixbuf == 0) {
		return W_ERROR_NO_HANDLES;
	} else {
		return W_TRUE;
...
wresult w_surface_get_graphics(w_surface *bitmap, w_graphics *graphics) {
	...
	if (graphics == 0)
		return W_ERROR_NULL_ARGUMENT;
	cairo_t *cairo = cairo_create(_W_SURFACE(bitmap)->surface);
	if (cairo == 0) {
		return W_ERROR_NO_HANDLES;
	}
