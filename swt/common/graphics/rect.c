...
wbool w_rect_contains(w_rect *r, int x, int y) {
	return (x >= r->x) && (y >= r->y) && x < (r->x + r->width)
			&& y < (r->y + r->height);
}
wbool w_rect_equals(w_rect *r, w_rect *rect) {
