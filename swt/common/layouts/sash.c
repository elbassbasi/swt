...
void w_layout_sash_do_layout(w_layout *_layout, w_composite *composite, wbool flushCache);
...
int w_layout_sash_default_selection(w_widget *widget, w_event *event){
	...
	wbool correction = W_FALSE, doit = W_TRUE;
	...
	if (w_widget_get_style(widget) & W_VERTICAL) {
		...
		w_control_new_layout_data(c1,(void**)&d,
				sizeof(w_layout_sash_data));
		if (d != 0) {
			d->weight = ((b1.width << 16) + area.width - 1) / area.width;
		}
		w_control_new_layout_data(c2,(void**)&d,
				sizeof(w_layout_sash_data));
		if (d != 0) {
...
void w_layout_sash_set_weights(w_layout_sash *layout, w_composite *composite,
	...
	while (w_iterator_next(&children, &c)) {
		if (i < count) {
			w_control_new_layout_data(c,(void**)&d, sizeof(w_layout_sash_data));
			if (d != 0) {
...
void w_layout_sash_do_layout(w_layout *_layout, struct w_composite *composite, wbool flushCache) {
			...
			if (w_widget_class_id(W_WIDGET(c)) != _W_CLASS_SASH) {
				if (length > 0) {
					if ((length - 1) < layout->count) {
						w_control_set_bounds_(
								W_CONTROL(&layout->sashes[length - 1]), x,
								area.y, sashwidth, area.height);
					}
					x += sashwidth;
				}
				w_control_get_layout_data(c,(void**)&d);
				...
				width = (int) (weight * (area.width - layout->count * sashwidth)
						/ total);
				w_control_set_bounds_(W_CONTROL(c), x, area.y, width,
						area.height);
				x += width;
				length++;
			}
		...
		while (w_iterator_next(&children, &c)) {
			if (w_widget_class_id(W_WIDGET(c)) != _W_CLASS_SASH) {
				if (length > 0) {
					if ((length - 1) < layout->count) {
						w_control_set_bounds_(
								W_CONTROL(&layout->sashes[length - 1]), area.x,
								y, area.width, sashwidth);
					}
					y += sashwidth;
				}
				w_control_get_layout_data(c,(void**)&d);
				...
				w_control_set_bounds_(c, area.x, y, area.width, height);
				y += height;
				length++;
			}
		}
