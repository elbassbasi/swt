...
void w_layout_fill_compute_child_size(w_control *control, w_size *size,
		int wHint, int hHint, wbool flushCache) {
	if (wHint == W_DEFAULT && hHint == W_DEFAULT) {
		...
		int w = wHint == W_DEFAULT ? wHint : w_max(0, wHint - trim.width);
		int h = hHint == W_DEFAULT ? hHint : w_max(0, hHint - trim.height);
...
void w_layout_fill_compute_size(w_layout *layout, struct w_composite *composite,
	...
	while (w_iterator_next(&children, &child)) {
		if (child != 0) {
			...
			maxWidth = w_max(maxWidth, size->width);
			maxHeight = w_max(maxHeight, size->height);
		}
