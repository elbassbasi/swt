...
void w_layout_grid_do_layout_0(w_layout_grid *layout, w_composite *composite,
		int move, w_rect *r, w_size *sz, wbool flushCache);
/*
 * public function
 */
...
w_grid* w_grid_alloc(int children_count, int columnCount) {
	...
	if (grid == 0)
		return 0;
	grid->columns = (w_grid_column*) ((void*) grid + grid_size + children_size);
	grid->rows = (w_grid_row*) ((void*) grid + grid_size + children_size
			+ column_size);
...
void w_grid_compute_sizes(w_grid *grid, wbool flushCache) {
		...
		if (child->data.grabExcessHorizontalSpace
			...
			if (child->data.cacheWidth < child->data.minimumWidth) {
				...
				w_grid_data_compute_size(child,
						w_max(0, (child->data.minimumWidth - trim)),
						child->data.heightHint, flushCache);
			}
		}
		if (child->data.grabExcessVerticalSpace
				&& child->data.minimumHeight > 0) {
			child->data.cacheHeight = w_max(child->data.cacheHeight,
	...
	*row = &grid->rows[grid->currentRow];
	...
	for (size_t i = 0; i < columnCount; i++) {
		if (grid->currentControl < grid->children_count) {
			...
			size_t hSpan = w_max(1, w_min(c->data.horizontalSpan, columnCount));
			size_t vSpan = w_max(1, c->data.verticalSpan);
			size_t column = i;
...
int w_grid_column_next(w_grid *grid, w_grid_column **column,
		w_grid_control **control, wbool first) {
	if (grid->currentColumn < grid->columnCount) {
		...
		int hSpan = w_max(1, w_min(c->data.horizontalSpan, grid->columnCount));
		int vSpan = w_max(1, c->data.verticalSpan);
		if (first) {
...
void w_grid_column_with(w_grid *grid, w_layout_grid *layout, w_rect *r) {
	...
	while ((i = w_grid_row_next(grid, &row)) >= 0) {
		while ((j = w_grid_column_next(grid, &column, &child, W_TRUE)) >= 0) {
			if (child != 0) {
				int hSpan = w_max(1,
						w_min(child->data.horizontalSpan, columnCount));
				if (hSpan == 1) {
					int w = child->data.cacheWidth
							+ child->data.horizontalIndent;
					column->widths = w_max(column->widths, w);
					if (child->data.grabExcessHorizontalSpace) {
					...
					if (!child->data.grabExcessHorizontalSpace
						...
						column->minWidths = w_max(column->minWidths, w);
					}
				}
			}
		}
				...
				if (hSpan > 1) {
					...
					if (w > 0) {
						if (layout->makeColumnsEqualWidth) {
							...
							for (int k = 0; k < hSpan; k++) {
								columns[last = j - k].widths = w_max(equalWidth,
										columns[j - k].widths);
							}
							if (last > -1)
	...
	if (layout->makeColumnsEqualWidth) {
		...
		for (int i = 0; i < columnCount; i++) {
			minColumnWidth = w_max(minColumnWidth, columns[i].minWidths);
			columnWidth = w_max(columnWidth, columns[i].widths);
		}
		columnWidth =
				r->width == W_DEFAULT || expandCount == 0 ?
						columnWidth :
						w_max(minColumnWidth, availableWidth / columnCount);
		for (int i = 0; i < columnCount; i++) {
			...
			while (totalWidth != availableWidth) {
				...
				while ((i = w_grid_row_next(grid, &row)) >= 0) {
					while ((j = w_grid_column_next(grid, &column, &child,
						...
						if (child != 0) {
							int hSpan = w_max(1,
									w_min(child->data.horizontalSpan,
											columnCount));
							if (hSpan > 1) {
								if (!child->data.grabExcessHorizontalSpace
										|| child->data.minimumWidth != 0) {
					...
					if (child->data.heightHint == W_DEFAULT) {
						...
						if ((currentWidth != child->data.cacheWidth
							...
							child->data.cacheWidth = child->data.cacheHeight =
									W_DEFAULT;
							w_grid_data_compute_size(child,
									w_max(0, currentWidth - trim),
									child->data.heightHint, W_FALSE);
							if (child->data.grabExcessVerticalSpace
									&& child->data.minimumHeight > 0) {
								child->data.cacheHeight = w_max(
...
void w_grid_row_heights(w_grid *grid, w_layout_grid *layout, w_rect *r) {
	...
	while ((i = w_grid_row_next(grid, &row)) >= 0) {
		while ((j = w_grid_column_next(grid, &column, &child, W_TRUE)) >= 0) {
			if (child != 0) {
				int vSpan = w_max(1,
						w_min(child->data.verticalSpan, grid->rowCount));
				if (vSpan == 1) {
					int h = child->data.cacheHeight
							+ child->data.verticalIndent;
					row->heights = w_max(row->heights, h);
					if (child->data.grabExcessVerticalSpace) {
						if (!row->expandRow)
							expandCount++;
					...
					if (!child->data.grabExcessVerticalSpace
						...
						row->minHeights = w_max(row->minHeights, h);
					}
				}
			}
		}
	...
	while ((i = w_grid_row_next(grid, &row)) >= 0) {
		...
		while ((j = w_grid_column_next(grid, &column, &child, W_TRUE)) >= 0) {
			if (child != 0) {
				int hSpan = w_max(1,
						w_min(child->data.horizontalSpan, columnCount));
				int vSpan = w_max(1, child->data.verticalSpan);
				...
				int childWidth = w_min(child->data.cacheWidth, cellWidth);
				...
				int childHeight = w_min(child->data.cacheHeight, cellHeight);
