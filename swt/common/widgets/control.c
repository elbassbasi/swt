...
wresult w_control_get_bounds(w_control *control,w_point* location,w_size* size) {
	wresult result = W_WIDGET_CHECK0(control);
	if (result > 0) {
...
wresult w_control_set_bounds(w_control *control,w_point* location,w_size* size) {
	wresult result = W_WIDGET_CHECK0(control);
	if (result > 0) {
...
wresult w_control_pack(w_control *control, wbool changed) {
	wresult result = W_WIDGET_CHECK0(control);
	if (result > 0) {
...
wresult w_control_get_tooltip_text(w_control *control, w_alloc alloc,void* user_data) {
	wresult result = W_WIDGET_CHECK0(control);
	if (result > 0) {
		return W_CONTROL_GET_CLASS(control)->get_toolTip_text(control, alloc,user_data);
	} else {
		return result;
	}
}
...
wresult w_control_traverse(w_control *control, int traversal, w_event_key *event) {
	wresult result = W_WIDGET_CHECK0(control);
	if (result > 0) {
...
wresult w_control_set_tab(w_control *control, wbool tab) {
	wresult result = W_WIDGET_CHECK0(control);
	if (result > 0) {
