...
SWT_PUBLIC wresult w_path_create_from_path(w_path *path, w_path *frompath, float flatness);
SWT_PUBLIC wresult w_path_create_from_data(w_path *path, size_t length, char *types,
		float *pt);
...
SWT_PUBLIC wresult w_path_add_string(w_path *path, const char *string, int length, int enc,
...
SWT_PUBLIC wresult w_path_contains(w_path *path, w_pointf *pt, w_graphics *gc,
		int outline);
SWT_PUBLIC wresult w_path_cubicto(w_path *path, float cx1, float cy1, float cx2, float cy2,
		float x, float y);
...
SWT_PUBLIC wresult w_path_quadto(w_path *path, float cx, float cy, float x, float y);
