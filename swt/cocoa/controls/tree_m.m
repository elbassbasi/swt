...
- (void)outlineViewSelectionDidChange:(NSNotification *)notification{
    ...
    _w_widget_send_msg(widget, self, _NS_outlineViewSelectionDidChange, item, 0, 0);
}
- (void)outlineViewColumnDidMove:(NSNotification *)notification{
    _w_widget_send_msg(widget, self, _NS_outlineViewColumnDidMove, notification, 0, 0);
}
- (void)outlineViewColumnDidResize:(NSNotification *)notification{
    _w_widget_send_msg(widget, self, _NS_outlineViewColumnDidResize, notification, 0, 0);
}
- (void)outlineViewSelectionIsChanging:(NSNotification *)notification{
    _w_widget_send_msg(widget, self, _NS_outlineViewSelectionIsChanging, notification, 0, 0);
}
- (void)outlineViewItemWillExpand:(NSNotification *)notification{
    NSObject * item = [notification.userInfo objectForKey:@"NSObject"];
    _w_widget_send_msg(widget, self, _NS_outlineViewItemWillExpand, item, 0, 0);
}
- (void)outlineViewItemDidExpand:(NSNotification *)notification{
    SWTTreeItem * item =(SWTTreeItem *) [notification.userInfo objectForKey:@"NSObject"];
    _w_widget_send_msg(widget, self, _NS_outlineViewItemDidExpand, item, 0, 0);
    if(item != nil){
...
- (void)outlineViewItemWillCollapse:(NSNotification *)notification{
    _w_widget_send_msg(widget, self, _NS_outlineViewItemWillCollapse, notification, 0, 0);
}
- (void)outlineViewItemDidCollapse:(NSNotification *)notification{
    _w_widget_send_msg(widget, self, _NS_outlineViewItemDidCollapse, notification, 0, 0);
}
@end
...
NSOutlineView* SWTOutlineView_new(void* userdata){
    ...
    _w_toolkit_registre_class(_NS_CLASS_OUTLINEVIEW,outlineView.class);
...
SWTTreeItem *SWTTreeItem_create(SWTOutlineView* outline,SWTTreeItem* parent,int index,const char* text){
...
void SWTTreeItem_setText(SWTTreeItem *item,const char* text){
    item->text = [NSString stringWithUTF8String:text];
}
