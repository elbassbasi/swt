...
public:
	...
	WFontData(const char *name) {
		w_fontdata_init(W_FONTDATA(this));
		w_fontdata_create(W_FONTDATA(this), name, 12, W_NORMAL);
	}
	/**
	...
	WFontData(const char *name, int height, int style) {
		w_fontdata_init(W_FONTDATA(this));
		w_fontdata_create(W_FONTDATA(this), name, height, style);
	}
	/**
	...
	WString GetLocale() {
		WString str;
		w_fontdata_get_locale(W_FONTDATA(this),w_alloc_string_ref, &str.ref);
		return str;
	}
	/**
	 * Returns the locale of the receiver.
	 ...
	 * @since 3.0
	 */
	size_t GetLocale(char *locale, size_t size) {
		w_alloc_buffer buffer;
		buffer.buffer = locale;
		buffer.size = size;
		buffer.total_size = 0;
		w_fontdata_get_locale(W_FONTDATA(this), w_alloc_buffer_copy, &buffer);
	...
	bool Set(const char *name, int height, int style) {
		return w_fontdata_set(W_FONTDATA(this), name, height, style) > 0;
	}
	/**
	...
	bool SetLocale(const char *locale) {
		return w_fontdata_set_locale(W_FONTDATA(this), locale) > 0;
	}
	/**
	...
	bool SetName(const char *name) {
		return w_fontdata_set_name(W_FONTDATA(this), name) > 0;
	}
	/**
...
public:
	...
	bool Create(const char *name, int style, int size) {
		return w_font_create(W_FONT(this), name, style, size) > 0;
	}
	/**
