...
public:
	 ...
	 * @see Widget#getStyle
	 */
	bool Create(WMenuItem *parent, const char *text, int style, int index) {
		return w_menuitem_insert(W_MENUITEM(parent), W_MENUITEM(this), text,
				style, index) > 0;
	}
	/**
	 ...
	 */
	WMenuItem& GetItem(int index, WMenuItem &item) {
		w_menuitem_get_item(W_MENUITEM(this), index, W_MENUITEM(&item));
		return item;
	}
	/**
	 * Returns the item at the given, zero-relative index in the
	 ...
	 */
	void GetItems(WIterator<WMenuItem> &items) const {
		w_menuitem_get_items(W_MENUITEM(this), (w_iterator*) &items);
	 ...
	 */
	bool GetImage(WImage &image) const {
		return w_menuitem_get_image(W_MENUITEM(this), W_IMAGE(&image)) > 0;
	...
	bool GetParent(WMenuItem &parent) {
		return w_menuitem_get_parent(W_MENUITEM(this), W_MENUITEM(&parent));
	}
	/**
	 ...
	 */
	bool Remove() {
		return w_menuitem_remove(W_MENUITEM(this)) > 0;
	}
	/**
	 * remove and destroy the menu item from the this menu.
	 */
	bool RemoveItem(int index) {
		return w_menuitem_remove_item(W_MENUITEM(this), index) > 0;
	}
	/**
	 ...
	 */
	WMenuItem& SetAccelerator(int accelerator) {
		w_menuitem_set_accelerator(W_MENUITEM(this), accelerator);
		return *this;
	}
	/**
	...
	WMenuItem& SetEnabled(bool enabled) {
		w_menuitem_set_enabled(W_MENUITEM(this), enabled);
		return *this;
	}
	/**
	...
	WMenuItem& SetId(wushort id) {
		w_menuitem_set_id(W_MENUITEM(this), id);
		return *this;
	}
	/**
	...
	WMenuItem& SetImage(const WImage &image) {
		w_menuitem_set_image(W_MENUITEM(this), W_IMAGE(&image));
		return *this;
	}
	/**
	 * Sets the selection state of the receiver.
	...
	WMenuItem& SetSelection(bool selected) {
		w_menuitem_set_selection(W_MENUITEM(this), selected);
		return *this;
	}
public:
...
public:
	WMenuItem& Insert(WMenuItem &item, const char *text, int style, int index) {
		w_menuitem_insert(W_MENUITEM(this), W_MENUITEM(&item), text, style,
				index);
		return item;
	}
...
public:
	...
	void GetBounds(WRect &bounds) {
		w_menu_get_bounds(W_MENU(this), (w_rect*) &bounds);
	}
	/**
	 ...
	 */
	WControl* GetParent() {
		return (WControl*) w_menu_get_parent(W_MENU(this));
	}
	WShell* GetShell() {
		w_control *control = w_menu_get_parent(W_MENU(this));
		WShell *shell;
		w_control_get_shell(control, (w_shell**) &shell);
		return shell;
	}
	/**
	...
	void SetOrientation(int orientation) {
		w_menu_set_orientation(W_MENU(this), orientation);
	}
	/**
	...
	bool SetVisible(bool visible) {
		return w_menu_set_visible(W_MENU(this), visible) > 0;
	}
private:
