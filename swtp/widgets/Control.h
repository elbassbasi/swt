...
public:
	...
	virtual WResult Create(WComposite *parent, wuint64 style,
			WToolkit *toolkit)=0;
	bool Create(WComposite *parent, wuint64 style) {
		return Create(parent, style, 0) > 0;
	}
	...
	WString GetToolTipText() {
		WString str;
		_get_toolTip_text(w_alloc_string_ref, &str.ref);
		return str;
	}
	/**
	 ...
	 */
	void SetToolTipText(const char *string,size_t length) {
		_set_toolTip_text(string,length);
	}
	/**
	 * Sets the receiver's tool tip text to the argument, which
	 * may be null indicating that the default tool tip for the
...
public:
	...
	WResult _get_bounds(WPoint* location,WSize* size) {
		return w_control_get_bounds(W_CONTROL(this),(w_point*)location,(w_size*)size);
	}
	WResult _set_bounds(const WPoint* location,const WSize* size) {
		return w_control_set_bounds(W_CONTROL(this),(w_point*)location,(w_size*)size);
	}
	...
	WResult _get_toolTip_text(w_alloc alloc,void* user_data) {
		return w_control_get_tooltip_text(W_CONTROL(this), alloc,user_data);
	}
	WResult _set_toolTip_text(const char *text,size_t length) {
		return w_control_set_tooltip_text(W_CONTROL(this), text,length);
	}
	...
	WResult _traverse(int traversal, WKeyEvent *event) {
		return w_control_traverse(W_CONTROL(this), traversal, (w_event_key*) event);
	}
	WResult _update() {
